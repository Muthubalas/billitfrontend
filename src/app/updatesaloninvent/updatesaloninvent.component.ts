...
@Component({
  selector: 'app-updatesaloninvent',
  templateUrl: './updatesaloninvent.component.html',
  styleUrls: ['./updatesaloninvent.component.css']
})
export class UpdatesaloninventComponent implements OnInit {
  getpro: any;
  select: any;
  ...
  private dialog:MatDialogRef<UpdatesaloninventComponent>,private router:Router) { }
...
this.salon.controls['description'].setValue(this.edit.Product_description);
  ...
  getproductlist(){
   ...
   quantity(num:any){
    this.edit.Product_type=num.target.value;
   ...
   addinvent(){
    ...
    && this.salon.value.quantity != null ){
    const url = id._id;
